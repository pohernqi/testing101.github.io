...
function getDeclarationItems() {
    ...
    document.querySelectorAll('#declarations .declaration').forEach(declaration => {
        const description = declaration.querySelector('.desc')?.value || '';
        const type = declaration.querySelector('.type')?.value || '';
        const specifyOthers = declaration.querySelector('.others-type')?.value || '';
        const declarationQuantity = declaration.querySelector('.quantity')?.value || '';
...
document.getElementById('visitorForm').addEventListener('submit', function (e) {
    ...
    const declarationItems = getDeclarationItems(); // [{description, type}, ...]


    if (isGroup) {
        const numPeople = parseInt(document.getElementById('numPeople').value);
        const vehicleNo = document.getElementById('sharedVehicleNo').value;
        ...
        const timeOutExpectedInput = document.getElementById('timeOutExpected').value;
        //const declarations = getDeclarationString();

        // Convert datetime-local input to formatted string
        const timeOutExpected = timeOutExpectedInput
            ? new Date(timeOutExpectedInput).toLocaleString('en-GB', { hour12: false })
            : ''; // Handle empty case if needed

        // Collect multiple entries into arrays
        let names = [], nationalities = [], icNumbers = [];
        ...
        const nationality = document.getElementById('personalNationality')?.value || '';
        const icNumber = document.getElementById('personalIC')?.value || '';
        const vehicleNo = document.getElementById('sharedVehicleNo')?.value || '';
        const purpose = document.getElementById('sharedPurpose')?.value || '';
        const hostName = document.getElementById('hostName')?.value || '';
        const timeOutExpectedInput = document.getElementById('timeOutExpected')?.value || '';
        ...
        console.log('Personal Info:', { name, nationality, icNumber, vehicleNo, purpose, hostName, timeOutExpected });
