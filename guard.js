        ...
        rows.forEach((row, index) => {
            ...
            const decisionInCellContent =
                row.decisionIn?.toLowerCase() === 'pending'
                    ...
                    <label><input type="radio" name="decisionIn${index}" value="Approve">✔️</label>
                    <label><input type="radio" name="decisionIn${index}" value="Reject">❌</label>
                     </div>
                `
                    : row.decisionIn || '';
            ...
            const decisionOutCellContent =
                row.decisionOut?.toLowerCase() === 'pending'
                    ? `
                    <div class="vertical-radios">
                    <label><input type="radio" name="decisionOut${index}" value="Approve">✔️</label>
                    <label><input type="radio" name="decisionOut${index}" value="Reject">❌</label>
                    </div>
                `
                    : row.decisionOut || '';
    ...
    function applyDayFilter() {
        ...
        for (let i = 0; i < rows.length; i++) {
            const timestampCell = rows[i].cells[0].textContent;
            const isVisible = isDateMatch(timestampCell, targetDateStr);
